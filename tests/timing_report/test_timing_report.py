...
sTestDir = os.path.dirname(__file__) + '/parse/vivado/'
...
class Test(unittest.TestCase):
    ...
    def test_new_timing_report(self):
        ...
        dTimingReport = vivado.parse_timing_report(lHoldFile)

        oTr = tr.new(dTimingReport[0])

        self.assertEqual(0.245, oTr.crpr)
        self.assertEqual(0.272, oTr.get_slack())
        self.assertEqual(0.000, oTr.launch_edge)
        self.assertEqual(0.000, oTr.capture_edge)
        self.assertEqual(1.0, oTr.output_delay)
        self.assertEqual('Fast Process Corner', oTr.process_corner)
        self.assertTrue(isinstance(oTr, tr.HoldTimingReport))
